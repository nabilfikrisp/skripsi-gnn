...
class InvertedResidual(nn.Module):
    def __init__(
        ...
        self.mlp = MLP(
            in_features=dim, hidden_features=mlp_hidden_dim, drop=drop, mid_conv=True
        )
...
class FFN(nn.Module):
    def __init__(
        self, in_features, hidden_features=None, out_features=None, drop_path=0.0
    ):
        super().__init__()
...
class MobileViG(torch.nn.Module):
        ...
        for i in range(len(local_blocks)):
            if i > 0:
                self.local_backbone.append(
                    Downsample(local_channels[i - 1], local_channels[i])
                )
            for _ in range(local_blocks[i]):
                self.local_backbone.append(
                    InvertedResidual(
                        dim=local_channels[i], mlp_ratio=4, drop_path=dpr[dpr_idx]
                    )
                )
                dpr_idx += 1
        self.local_backbone.append(
        ...
        for i in range(len(global_blocks)):
            if i > 0:
                self.backbone.append(
                    Downsample(global_channels[i - 1], global_channels[i])
                )
    ...
    def forward(self, inputs):
        ...
        if self.distillation:
            x = (
                self.head(x).squeeze(-1).squeeze(-1),
                self.dist_head(x).squeeze(-1).squeeze(-1),
            )
...
# @register_model
def mobilevig_ti(pretrained=False, num_classes=10, **kwargs):
    model = MobileViG(
        local_blocks=[2, 2, 6],
    ...
    model.default_cfg = default_cfgs["mobilevig"]  # type: ignore
    return model
